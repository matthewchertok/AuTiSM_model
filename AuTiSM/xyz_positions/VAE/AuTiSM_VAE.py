...
class TemporalEncoder(nn.Module):
    """Temporal encoder that processes sequences of latent vectors into a latent distribution.

    This module takes a sequence of latent vectors (latent space A) that were generated from
    position encodings over multiple frames, and encodes them into a latent distribution
    (mean and log variance) in latent space B using a GRU network and fully connected layers.
    ...
    Parameters:
        ...
        fc_mu (nn.Linear): Fully connected layer to compute mean of the latent distribution.
        fc_logvar (nn.Linear): Fully connected layer to compute log variance of the latent distribution.
    ...
    Returns:
        (torch.Tensor, torch.Tensor): Mean and log variance of the latent distribution.
            Each has shape (batch_size, latent_dim_B)
    """
    ...
    def __init__(self, input_size=64, latent_dim_B=8):
        ...
        self.fc_mu = nn.Linear(latent_dim_B, latent_dim_B)
        self.fc_logvar = nn.Linear(latent_dim_B, latent_dim_B)

    def forward(self, latent_A_seq):
        _, h_n = self.gru(latent_A_seq)
        h_n = h_n.squeeze(0)  # Shape: [batch_size, latent_dim_B]
        mu = self.fc_mu(h_n)
...
class AuTiSM_Model(nn.Module):
    ...
    This model implements a hierarchical variational autoencoder architecture that processes particle position data
    ...
    Architecture:
        ...
        - Sequence-level variational encoding/decoding: Handles temporal relationships between frames
    ...
    Returns:
        reconstructed_x: Tensor of same shape as input, containing reconstructed particle positions
            across all frames in the sequence
        mu: Mean of the latent distribution from the temporal encoder
        logvar: Log variance of the latent distribution from the temporal encoder
    ...
    def forward(self, x):
        ...
        latent_A_seq = torch.stack(latent_A_list, dim=1)  # Shape: [batch_size, M, latent_dim_A]
        mu, logvar = self.sequence_encoder(latent_A_seq)
        latent_B = self.reparameterize(mu, logvar)
        ...
        # Apply tanh activation to ensure output is in [-1, 1]
        # since the input data is normalized to [-1, 1]
