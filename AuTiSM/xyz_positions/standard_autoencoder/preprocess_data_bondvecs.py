...
def process_file(
    ...
    for i, sublist in enumerate(positions_all_frames):
        ...
        # Compute bond vectors for each frame
        bond_vectors = backbone_positions_all_frames[:, 1:] - backbone_positions_all_frames[:, :-1]

        # Find the maximum absolute value across all bonds and dimensions for normalization
        max_bond_distance = np.max(np.abs(bond_vectors))

        # Normalize all bond vectors by the same value to preserve relative distances
        normalized_bond_vectors = bond_vectors / max_bond_distance
        
        # Create reversed bond vectors for data augmentation
        normalized_bond_vectors_reversed = -normalized_bond_vectors[::-1]
        ...
        results.append(
            {
                ...
                "asphericity": b,
                "frames": frames,
                "bond_vectors" : normalized_bond_vectors,
                "bond_vectors_reversed" : normalized_bond_vectors_reversed,  # include reversed positions for data augmentation
                "f": float(f),
                "beta": float(beta),
                "kappa": float(kappa),
