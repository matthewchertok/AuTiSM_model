...
def loss_function(reconstructed_x, x, mu, logvar, kl_weight=0.1):
...
# Define kl_weights as a torch array
kl_weights = torch.cat((torch.linspace(0, 0.1, steps=50), torch.full((50,), 0.1)))
...
for epoch in range(num_epochs):
    ...
    for batch in dataloader:
        ...
        loss = loss_function(
            reconstructed_x, batch.float(), mu, logvar, kl_weight=kl_weight
        )
    ...
    print(
        f"Epoch [{epoch+1}/{num_epochs}], Loss: {avg_loss:.4f}, KL Weight: {kl_weight:.4f}"
    )
...
# Save model weights
filename = f"AuTiSM_VAE_latentA={latent_dim_A}_latentB={latent_dim_B}.pth"
torch.save(model.state_dict(), filename)
print("Model training complete. Weights saved to file:", filename)
