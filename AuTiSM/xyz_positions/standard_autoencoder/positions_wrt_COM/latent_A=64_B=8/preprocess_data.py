...
def process_file(
    ...
    for i, sublist in enumerate(positions_all_frames):
        ...
        # normalize the relative positions to the range [-1,1]
        # Find max absolute value across all dimensions for each frame
        max_abs_vals = np.max(np.abs(ri_minus_rcom), axis=(1,2))
        # Normalize each frame by its max value
        ri_minus_rcom = ri_minus_rcom / max_abs_vals[:, np.newaxis, np.newaxis]
