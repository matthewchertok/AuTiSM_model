...
parser = argparse.ArgumentParser(
    description="Process GSD files to compute pairwise distances and apply 2D convolution."
)
parser.add_argument(
    "--window_length",
    type=int,
    default=50,
    help="Length of the window for splitting frames into sublists.",
)
args = parser.parse_args()
window_length = args.window_length


def process_file(file, last_frames_frac, reversible_bonding=True, n_backbone=100):
    ...
    with gsd.hoomd.open(filepath) as traj:
        ...
        frame_indices = [
            (i * sublist_length, (i + 1) * sublist_length) for i in range(n_sublists)
        ]
    ...
    for i, sublist in enumerate(positions_all_frames):
        ...
        normalized_x = (dx - np.min(dx, axis=(1, 2), keepdims=True)) / (
            np.max(dx, axis=(1, 2), keepdims=True)
            - np.min(dx, axis=(1, 2), keepdims=True)
        )
        normalized_y = (dy - np.min(dy, axis=(1, 2), keepdims=True)) / (
            np.max(dy, axis=(1, 2), keepdims=True)
            - np.min(dy, axis=(1, 2), keepdims=True)
        )
        normalized_z = (dz - np.min(dz, axis=(1, 2), keepdims=True)) / (
            np.max(dz, axis=(1, 2), keepdims=True)
            - np.min(dz, axis=(1, 2), keepdims=True)
        )
        ...
        evecs_x = (evecs_x - np.min(evecs_x, axis=(1, 2), keepdims=True)) / (
            np.max(evecs_x, axis=(1, 2), keepdims=True)
            - np.min(evecs_x, axis=(1, 2), keepdims=True)
        )
        evecs_y = (evecs_y - np.min(evecs_y, axis=(1, 2), keepdims=True)) / (
            np.max(evecs_y, axis=(1, 2), keepdims=True)
            - np.min(evecs_y, axis=(1, 2), keepdims=True)
        )
        evecs_z = (evecs_z - np.min(evecs_z, axis=(1, 2), keepdims=True)) / (
            np.max(evecs_z, axis=(1, 2), keepdims=True)
            - np.min(evecs_z, axis=(1, 2), keepdims=True)
        )

        # Stack the eigenvectors into 3 channels
        pdist_evecs = np.stack([evecs_x, evecs_y, evecs_z], axis=-1)

        # Now get normalized pairwise distances for each frame

        # Get maximum range across all dimensions for each frame to preserve aspect ratio
        max_range_x = np.max(dx, axis=(1, 2), keepdims=True) - np.min(
            dx, axis=(1, 2), keepdims=True
        )
        max_range_y = np.max(dy, axis=(1, 2), keepdims=True) - np.min(
            dy, axis=(1, 2), keepdims=True
        )
        max_range_z = np.max(dz, axis=(1, 2), keepdims=True) - np.min(
            dz, axis=(1, 2), keepdims=True
        )
        max_range = np.maximum.reduce([max_range_x, max_range_y, max_range_z])

        # Normalize while preserving aspect ratio. Range is now [0, 1].
        pdist_x = (dx - np.min(dx, axis=(1, 2), keepdims=True)) / max_range
        pdist_y = (dy - np.min(dy, axis=(1, 2), keepdims=True)) / max_range
        pdist_z = (dz - np.min(dz, axis=(1, 2), keepdims=True)) / max_range

        # Stack the normalized distances into 3 channels
        pdist = np.stack([pdist_x, pdist_y, pdist_z], axis=-1)
